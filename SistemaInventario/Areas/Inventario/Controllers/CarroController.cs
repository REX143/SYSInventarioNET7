...
namespace SistemaInventario.Areas.Inventario.Controllers
{
    ...
    public class CarroController : Controller
    {
        ...
        public async Task<IActionResult> Index()
        {
            ...
            carroCompraVM.Orden = new Modelos.Admin.Orden();
            carroCompraVM.CarroCompraLista = await _unidadTrabajo.CarroCompra
                                                                 .ObtenerTodos(u => u.UsuarioAplicacionId == claim!.Value,
            ...
            {
                lista.Precio = lista.Producto.Precio; // Siempre mostrar precio actual
            ...
            foreach (var lista in carroCompraVM.CarroCompraLista)
            {
                ...
                if (lista.Cantidad > producto.Cantidad)
                {
                    TempData[DS.Error] = "La cantidad del producto " + lista.Producto.Descripcion +
                                         "es mayor al stock disponible actual (" + producto.Cantidad + ")";
                    return RedirectToAction("Index");
                }
            }
        ...
        public async Task<IActionResult> Proceder(CarroCompraVM carroCompraVM)
        {
            ...
            // Interacción con pasarela de pago Stripe
            ...
            foreach (var lista in carroCompraVM.CarroCompraLista)
            {
                var sessionLineItem = new SessionLineItemOptions()
                {
                    ...
                    {
                        UnitAmount = (long)(lista.Precio * 100), // asi pide stripe
                        Currency = "usd",
            ...
            await _unidadTrabajo.Guardar();
            Response.Headers.Add("Location", session.Url); // redirección a stripe
            return new StatusCodeResult(303);

            //return RedirectToAction("OrdenConfirmacion", new { id = carroCompraVM.Orden.Id}); ya lo hace stripe a traves de options.SuccessUrl
        }
        ...
        public async Task<IActionResult> OrdenConfirmacion(int id)
        {
            ...
            if (session.PaymentStatus.ToLower() == "paid")
            {
                ...
                // Actualizar stock de la bodega de ventas
                var compania = await _unidadTrabajo.Compania.ObtenerPrimero();
                foreach (var lista in carroCompra)
                {
                    var bodegProducto = new BodegaProducto();
                    bodegProducto = await _unidadTrabajo.BodegaProducto.ObtenerPrimero(b => b.ProductoId == lista.ProductoId
                                                                                       && b.BodegaId == compania.BodegaVentaId);
                    await _unidadTrabajo.KardexInventario.RegistrarKardex(bodegProducto.Id, "Salida",
                                                                          "Venta - Orden #" + id, bodegProducto.Cantidad,
                                                                          lista.Cantidad, orden.UsuarioAplicacionId);
                    bodegProducto.Cantidad -= lista.Cantidad;
                    await _unidadTrabajo.Guardar();
                }
            }

            //Borramos el carro de compras y la sesion           
